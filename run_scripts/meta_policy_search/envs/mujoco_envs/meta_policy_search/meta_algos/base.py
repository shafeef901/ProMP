...
class MAMLAlgo(MetaAlgo):
    ...
    def _make_input_placeholders(self, prefix=''):
        ...
        for task_id in range(self.meta_batch_size):
            # observation ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.policy.obs_dim], name='obs' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s'%(prefix, task_id, 'observations')] = ph
            obs_phs.append(ph)

            # action ph
            ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.policy.action_dim], name='action' + '_' + prefix + '_' + str(task_id))
            ...
            for info_key, shape in dist_info_specs:
                ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None] + list(shape), name='%s_%s_%i' % (info_key, prefix, task_id))
